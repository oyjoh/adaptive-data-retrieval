...
class Chunk:
    def __init__(self, dataframe, layer, tree_depth, max_datapoints):
        # sw/ne = (lat, lon)
        self.sw = None
        self.ne = None
        ...
        # revisit
        while self.ds.size > max_datapoints:
            self.ds = self.ds[::2, ::2]
    ...
    def split(self):
        ...
        self.upper_left = Chunk(
            c_1, self.layer+1, self.tree_depth, self.max_datapoints)
        self.upper_right = Chunk(
            c_2, self.layer+1, self.tree_depth, self.max_datapoints)
        self.lower_left = Chunk(
            c_3, self.layer+1, self.tree_depth, self.max_datapoints)
        self.lower_right = Chunk(
            c_4, self.layer+1, self.tree_depth, self.max_datapoints)
    ...
    def save_netcdf(self):
        self.ds.to_netcdf(f'{self.get_bounds()}.nc')
...
class QuadTreeSimple:
    ...
    def __init__(self, max_chunk_size, original_file_size, dataframe, bounds):
        ...
        self.max_datapoints = self.ds.size * reduction_factor
        self.num_original_points = self.ds.size
    ...
    def create_tree(self):
        return Chunk(self.ds, 1, self.num_layers, self.max_datapoints)
